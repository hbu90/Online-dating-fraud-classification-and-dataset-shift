...
# Replaced the data.index in plt_recall etc. with range(1, len(series)+1)
# Removed the set_style part to do with latex
...
def style_axes(axes, periods=10):
    for i, ax in enumerate(axes):
        ...
        # Ticks
        ax.set_xticks(ticks = range(1, periods + 1))
...
def plot_f1(ax, data, alpha=1.0, neg=False, label=None, color='dodgerblue',
            marker='o'):
    if label is None:
        label = 'F1 (real)' if neg else 'F1 (scam)'
    color = '#BCDEFE' if neg else color
    series = data['f1_n'] if neg else data['f1']
    ax.plot(range(1, len(series)+1), series, label=label, alpha=alpha, marker=marker,
...
def plot_recall(ax, data, alpha=1.0, neg=False, color='red', marker='^'):
    ...
    label = 'Recall (real)' if neg else 'Recall (scam)'
    series = data['recall_n'] if neg else data['recall']
    ax.plot(range(1, len(series)+1), series, label=label, alpha=alpha,
...
def plot_precision(ax, data, alpha=1.0, neg=False, color='orange', marker='s'):
    ...
    label = 'Precision (real)' if neg else 'Precision (scam)'
...
def fill_under_f1(ax, data, alpha=1, neg=False):
    label = 'F1 (real)' if neg else 'F1 (scam)'
    series = data['f1_n'] if neg else data['f1']
    ax.fill_between(range(1, len(series)+1), series,
...
def plot_cv_mean(ax, data, alpha=1):
    ax.axhline(y=float(data), linestyle='--', linewidth=1, c='black',
               alpha=alpha, label='F1 (baseline classifier)')
